...
# df.to_csv(path_or_buf='t_data/followers_and_following.csv')


"""testing area
"""

followers = ['ricky', 'james', 'moses', 'winston', 'zack', 'joe']
following = ['ricky', 'ricardo', 'winston', 'zack', 'tommy']
# merge for single list of all unique accounts in following/followers
unique_accounts = list(set(followers + following))
...
# bool followers & following (1 == True)
bool_followers = []
for account in df['account']:
    if account in followers:
        bool_followers.append(1)
